...
int main() {
	int fd;
	size_t size;
	//Создаем симфольный массив где будет храниться имя файла FIFO
	char name[] = "Fifo.fifo";
	//ПОсле первого запуска программы необходимо закоментировапть процесс создания файла FIFO
...
*/
		...
		if (size != 11) {
			printf("Не получилось записать на 11 байт в FIFO\n");
			exit(-1);
		}
		if (close(fd)<0) {
